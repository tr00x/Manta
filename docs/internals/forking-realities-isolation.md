...
Closed set — every Bus tool is either allow-listed or reject-listed below;
an unlisted tool is a gap. Cross-reference: `packages/manta-bus/src/server.ts`
tool table.
...
| manta.lock                    | allow       | allow (soft-leak — below)  | known limitation                 |
...
| manta.zk_write                | allow       | allow (skill-restricted)   | manta-as-clone skill             |
| manta.para_append             | allow       | allow (skill-restricted)   | manta-as-clone skill             |
...
The table enumerates all 25 Bus tools. The 6 daemon/work control-plane tools —
`retask`/`pause`/`resume`/`feedback`/`enqueue_work` plus `request_task` — are
`allow` because the caller is the **main/orchestrator**, not a sibling clone, so
they carry no sibling-plagiarism risk; `request_task` is a daemon clone's own
idle-signal.
...
- **Lock owner-id leak.** `BusLockedError.ownerCloneId` reveals who holds a
  ...
  parent-repo path to probe siblings. PreToolUse hooks are the durable fix
  (not yet shipped).
- **Filesystem-level access.** A skill-violating clone could
  `cd ../clone-B && cat`. Skill discipline is the only current defense;
  filesystem hooks are the durable fix (not yet shipped).
- **`requesting_clone_id` is operator-trusted.** A malicious clone can lie
  about its own clone_id. Per-connection identity (transport-verified
  `requesting_clone_id`) is the durable fix (not yet shipped).

## How `cast_id` stamping is consumed elsewhere

- **Merge-review** with composite scoring reads `cast_id`-stamped broadcast
  events to build per-clone quality signals.
- **`tail`** uses `cast_id` + `cast_mode` from broadcast events to filter
  sibling visibility before delivering to a tailing peer.
- When more modes need partial-isolation policies, the helper can swap from
  `cast_mode === 'forking-realities'` to
  `ctx.casts.read(cast_id).policy.peer_messaging === 'denied'`.
- Durable hardening (not yet shipped): PreToolUse hooks for filesystem-level
  enforcement + per-connection identity for transport-verified
  `requesting_clone_id`.
