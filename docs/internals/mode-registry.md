...
- **Share bundles.** `manta share <cast-id>` includes `libraryMode` so the recipient's `manta install` knows which package to look for. Without the dual recording, the share bundle can only point at a built-in dispatcher and the library origin is lost.
...
- **Per-mode invariants and capability gating** — the snapshot/registry pair is the right place to attach mode-specific guards (e.g. "this library mode requires `forking-realities` style worktree isolation"). Future capability-gated modes will need this; the field already has a home (`LibraryModeEntry.invariants?: ModeInvariant[]`).
