...
- **Why:** the literal one-line goal "install on a clean machine → cast works" is **NOT** true for a non-repo dir; shipping `1.0.0` without stating this would be the exact advertised-but-absent lie-class RB2 exists to kill. `1.0.0` is justified *conditional on this precondition being documented* (plan-review A2). **Surfaced to user before Chunk 0.**
...
- `packages/manta-cli/package.json`: `name` `@manta/cli`→`manta`; remove `private` (or `false`); `version` `0.0.0`→`1.0.0`; add `engines.node >=20`; add `"prepublishOnly": "pnpm run build"` (S-PUB5); confirm `files: ["dist"]` will carry all bins after Chunk 2; keep `bin.manta`. Add `repository`/`homepage`/`keywords` (npm hygiene).
...
- **A4 — CHANGELOG `[1.0.0]` cut** (plan-review A4): cut a `[1.0.0]` section here (same "make it releasable" beat as the version bump; top entry is currently `[Unreleased]`, last versioned `[0.1.0]`). If you open `CHANGELOG.md`, also fix its stale "18 tools"→"25" in the same pass (S-DOC9 is otherwise out of scope — don't re-open the file twice).
- **Gate:** `npm publish --dry-run` shows `name: manta@1.0.0`, not-private; `manta --version` prints `1.0.0`; `pnpm gate` green; `@manta/e2e` still resolves the renamed dep with all `--filter`/bin-path references updated.
...
2. **`1.0.0` vs `0.1.0`** → **RESOLVED: `1.0.0`, conditional on D5's precondition being documented** (else `0.1.0` is the more honest signal). (plan-review A2.)
...
4. **CHANGELOG `[1.0.0]` cut** → **RESOLVED: Chunk 1** (same releasable-beat as the version bump; fix stale "18 tools" if the file is opened). (plan-review A4.)
