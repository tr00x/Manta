...
| `2026-05-08-phase-2b-bus-isolation.md` | **TBD** | Strategy 1+2 from `docs/research/phase-2-bus-isolation.md`: schema refine (forking-realities requires `metadata.cast_id`), `BusForkingIsolationError` class, `siblingsInSameForkingCast` helper, `manta.message` reject + `manta.broadcast` cast_id stamp, spawner `MANTA_BUS_PEER_SCOPE` env, `manta-as-clone` v0.0.3 forking-realities bullet, plus `task_contract.read` + `claim_work` hardening. Depends on 2a's manifest + `metadata.cast_mode`. |
