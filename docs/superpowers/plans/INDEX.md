...
| `2026-05-08-phase-2b-bus-isolation.md` | **Approved — pending execute** (reviewer iteration 1: 7 must-fix applied — git-author parsing, skill version bump v0.0.3→v0.0.4, dead-code removal, discriminated-union helper return type narrowing, contract/claim return-shape drift fixes, events.log open-string note, handler re-exports from `@manta/bus` index) | Strategy 1+2 from `docs/research/phase-2-bus-isolation.md`. Chunk 1: schema refine (forking-realities requires `metadata.cast_id`), `BusForkingIsolationError` class, `siblingsInSameForkingCast`/`crossCloneRead` helpers, `manta.message` reject + `manta.broadcast` cast_id-stamp, `task_contract.read` cross-clone reject (with optional `requesting_clone_id`), `claim_work` reject for FR clones, handler factories re-exported from `@manta/bus` index. Chunk 2: spawner `MANTA_BUS_PEER_SCOPE` env, `manta-as-clone` v0.0.4 + `manta-coordinate` v0.0.2, `docs/internals/forking-realities-isolation.md` closed-set table (18 tools), e2e integration test. Depends on 2a's manifest + `metadata.cast_mode`. |
| `2026-05-08-phase-2c-merge-review.md` | **TBD** | `runMergeReview` orchestrator hook, composite weighted scoring per `docs/research/phase-2-best-of-n-patterns.md` (coverage 0.30 / diff 0.20 / complexity 0.20 / type 0.15 / lint 0.15 + perf bonus 0.10), `.manta/config/scoring.json`, hard test gate, hybrid auto-merge with `autoMergeThreshold = Infinity` for Phase 2, ε=0.05 tie-band, `manta-merge-review` skill, `/manta promote <id>` slash command, graveyard helper, cross-candidate ZK harvest, `findFinalisedCasts` reading the cast manifest. |
| `2026-05-08-phase-2d-observability.md` | **TBD** | Tier 3-4 commands: `/manta tail <cloneId>` (polling `EventsLog.readSince`), `/manta inspect <cloneId>`, `/manta replay <castId>` (cast-scoped via manifest), `/manta audit <cloneId>`. Decision per research: shell out to `manta` CLI rather than add `manta.events_read` MCP tool. End-to-end forking-realities cast test (`MANTA_E2E=1` gated). |
