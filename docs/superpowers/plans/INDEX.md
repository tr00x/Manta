...
| `2026-05-08-phase-2a-forking-spawn.md` | **Approved — pending execute** (reviewer iteration 1 done; must-fix and cheap-advisory applied) | Spawn surface for forking-realities + cast manifest. Chunk 1: shared `state/canonicalize.ts` extraction + new `state/casts.ts` (`CastsStore`) + schema entries (`CastIdSchema`, `CastPolicySchema`, `CloneAssignmentSchema`, `CastManifestSchema`, `CreateCastInputSchema`) + `BusPaths.castsDir/castFile` + `BusContext.casts` + spawner `metadata.cast_mode` + `casts.create` (idempotent). Chunk 2: `forking-realities` allowlist + `RunCastOptions.cloneAssignments` + asymmetric Σ-budget gate + `--tasks <yaml\|json>` flag (`yaml@^2.6` new dep) + `tasks-file.ts` parser + `{APPROACH_HINT_BLOCK}` priming slot + mode-aware `castPolicy` replacing Chunk 1's stub. |
