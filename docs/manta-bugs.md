...
**Fix:** Open. Candidate: `runQualityGate` should mirror the canonical pre-merge gate — `pnpm install` (or reuse a warm store) + `pnpm build` the worktree, then `pnpm gate` (= `tsc -b` + lint + test) rather than ad-hoc `npx tsc --noEmit` + `npx vitest`. Until fixed, a `no_candidates_passed_gate` / `test_gate` verdict is NOT authoritative: the curator must re-run `pnpm gate` (after build) in each committed worktree before discarding a cast. See also 2a follow-up (scorer should also distinguish `clone_did_not_commit` from a true `test_gate` failure).
