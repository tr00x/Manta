...
- **bug #35 re-exposed (HIGH for large-project reliability):** `cast.ts:932` runs `collector.collect` for all candidates via `Promise.all` → now `prepareWorktreeForGate` fires **concurrent `pnpm install` across N worktrees on the shared `.pnpm` store** — exactly the `ERR_MODULE_NOT_FOUND` race `pnpm-workspace.yaml` documents as bug #35. The old gate never installed so this was latent; the fix surfaces it for forking casts with ≥2 candidates. Remediate: serialize `prepareWorktreeForGate` across candidates (merge-all path is already serial) or add a store mutex + `--frozen-lockfile`.
