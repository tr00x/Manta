...
### #66 — clone boot exceeds the 300s startup grace when the parent transcript is large → clones reaped before first heartbeat, casts fail to start late in a long session

**Discovered:** 2026-05-30, by curator. The RB#4b reliability cast failed TWICE in a row, both reaped with `startup grace 300457ms > 300000ms (no first heartbeat)` — the clone's `claude --print` process never reached its first `manta.*` bus call within the 5-min startup window. Crucially this happened LATE in a very long curator session (huge parent transcript), whereas every earlier cast this session (recon, RB#4a, RB#3, E, F — when the session/transcript was smaller) started fine. Machine load was moderate (2.30), ~15 node/claude procs — not obviously saturated. The correlation is parent-transcript SIZE: clone boot forks/inspects the parent JSONL, and a multi-MB transcript (this session grew large) makes cold-start slower; past some point boot+MCP-handshake+priming exceeds the 300s grace and the orchestrator reaps the clone before it can heartbeat.
**Severity:** Medium-High and **directly relevant to the product's headline use case** ("huge impact on large projects" = long sessions = large transcripts). If clones can't cold-start once the curator's session transcript is big enough, dogfooding stalls exactly when the project is most active. Even with the `--distill-threshold-bytes` fallback (over-threshold → boot WITHOUT inheritance), the size CHECK / fork path is apparently still slow enough to blow the grace, or another boot step (MCP connect, priming render, skill discovery) is the gate.

**Reproducer:** run a `manta cast` late in a long Claude Code session whose transcript is multi-MB; the spawned clone is reaped at ~300s with `no first heartbeat`, `outcome=fail`, worktree untouched.

**Root cause (to confirm):** unknown which boot step blows the budget. Candidates: (a) transcript fork/size-stat on a large JSONL is O(size) and slow; (b) `--distill-threshold-bytes` path still reads/copies before deciding; (c) MCP `manta-bus` handshake or priming/skill-discovery hangs under accumulated process load. Instrument the clone-spawner boot path with timestamps to find the gate. NOT yet diagnosed — needs a focused debug session (ideally with a SMALL parent transcript so casts actually start).

**Workaround (curator):** `--startup-grace-ms <ms>` flag EXISTS (manta.ts:192) — bump it (e.g. 600000) for casts spawned from a large-transcript session. Better: run reliability casts from a FRESH session (small transcript) where cold-start is fast. This bug is why RB#4b could not be completed by cast in the originating session and was deferred to a fresh session.

**Fix:** Open. Time-box the clone boot path; if transcript-fork is the gate, make the size check O(1) (stat, not read) and ensure the over-threshold fallback truly skips any O(size) work. Consider raising the default startup grace, OR emitting an early "booting" heartbeat from the spawner the instant the child process starts (before the clone's first bus call) so a slow-but-alive boot isn't misread as dead. Add a test with a large synthetic parent transcript asserting the clone still registers within grace.
...
**Fix:** **PARTIAL — data-loss guard FIXED** 2026-05-30, RB#4a cast `cast-1780170539399` (clone A), cherry-picked `e84e55c`. Clone refined the root cause: `allocateCloneIds` (cast.ts:85) already skips non-DEAD letters (bug #19) AND `Registry.register` rejects live-duplicate registration via atomic CAS (registry.ts:44) — so **no registry corruption / live-clone overwrite is actually possible** (my original "bug-hunt stole a LIVE recon-B" framing was wrong; recon-B had already gracefully died → its letter was legitimately free). The real residual harm: `addWorktree` force-`rm -rf`s an ORPHAN worktree dir on letter reuse, destroying any UNCOMMITTED work a *crashed* clone left (graceful-death clones commit to their branch → clean orphan → safe to reclaim). Fix: `addWorktree` now runs `git status --porcelain` on a pre-existing worktree dir and THROWS a clear operator error if dirty, instead of clobbering unsaved work; clean orphans still reclaim (no regression). Gate green 171/1462. **Structural follow-up (Open, not blocking v1):** cast-scoped worktree paths (`clone-<castId>-<L>`) to eliminate letter→dir aliasing entirely — clone honestly escalated this as out-of-package-scope (spans spawner/cast/recover/teardown call sites). For v1 the data-loss guard is sufficient: concurrent casts get disjoint letters (allocator + CAS), clean orphan reuse is safe, dirty orphan reuse fails loudly rather than silently. Track structural fix for a focused refactor cast post-publish.
...
**Severity:** High — the merge-scorer is a core forking-realities primitive, and its gate structurally false-negatives any branch subject to the pre-existing build-before-gate requirement (essentially every cast touching `manta-cli`). It makes `no_candidates_passed_gate` an untrustworthy verdict that the curator must override empirically on every cast, defeating the automated-selection value of merge-review. Distinct from the 2a `clone_did_not_commit` blind spot (where branches sat at base HEAD) — here the clones committed, and the gate itself is wrong.

**Reproducer:** in a freshly-`git worktree add`ed clone worktree with no `node_modules`/`dist` (the scorer's exact runtime), run the scorer's gate steps directly: `npx tsc --noEmit` reds with TS2307 "Cannot find module '@manta/bus'" (sibling `dist/index.d.ts` does not exist without a build; `tsc --noEmit` does not build project `references`), and/or `npx vitest run` trips bug #53's fresh-install heartbeat-hook red. Either disqualifies the candidate. The canonical `pnpm gate` (`pnpm build` first, then `tsc -b` + lint + test) is GREEN on the same tree.

**Root cause:** `packages/manta-cli/src/commands/cast.ts:1064-1075` (`runQualityGate`) runs, in the bare worktree: `git diff --stat HEAD` (hasDiff), then `npx tsc --noEmit` (line 1068), then `npx vitest run --reporter=dot` (line 1071). It never runs `pnpm install` + `pnpm build` to prepare the worktree, and it uses `tsc --noEmit` instead of the canonical `tsc -b` (so it neither finds pre-built sibling declarations nor builds the project references that would produce them). It also omits the lint dimension entirely. The scorer's "gate" is therefore a different, weaker, build-naive check than `pnpm gate`, and reds on the (pre-existing, by-design) build-before-typecheck property of the workspace.

**Fix:** **FIXED** 2026-05-30, front-E hardening cast `cast-1780169031321` (bug-hunt, clone B), cherry-picked to main as `34062c1`/amended. Clone root-caused DEEPER than this log: the Chunk 2/3/4 false-negatives came through the **forking-realities** gate in `merge-review-collector.ts` (writes `docs/merge-reviews/`), NOT the cited `cast.ts:1064 runQualityGate` (which serves the merge-all/refactor-wave path → `docs/merge-all-reports/`). Both gates shared the defect. Fix introduces a shared `prepareWorktreeForGate()` (exported from `merge-review-collector.ts`: `pnpm install --prefer-offline` + `pnpm -r --filter ./packages/* build`) called by BOTH paths before measuring, then canonical commands: `pnpm typecheck` (`tsc -b`, never `--noEmit`), `pnpm test` (root, not `pnpm -r test`), eslint scoped `packages/**/src/**/*.ts`. +5 regression tests pinning the command invariants. **Curator-verified independently: `pnpm gate` GREEN, 171 test files / 1459 tests, EXIT=0, zero regressions** (not trusting the clone's numeric claim — re-ran the full gate myself).

**Follow-ups (code-review FLAGs, → RB#4 reliability cast, NOT #63 re-openers — the core fix is correct & merged):**
- **bug #35 re-exposed (HIGH for large-project reliability): FIXED** 2026-05-30, RB#4a cast `cast-1780170539399`, cherry-picked `3ba3285`. `cast.ts:932` runs `collector.collect` for all candidates via `Promise.all` → `prepareWorktreeForGate` fired **concurrent `pnpm install` across N worktrees on the shared `.pnpm` store** (the `ERR_MODULE_NOT_FOUND` race `pnpm-workspace.yaml` documents as bug #35). Fix: a process-wide promise-chain mutex (`runSerialized`) serializes the install+build prepare across all candidates so no two run at once; the chain never breaks on a rejected run (each link swallows its outcome → no deadlock). Also switched install to `--frozen-lockfile --prefer-offline` (warm-store near-noop, can't rewrite the tracked lockfile → also closes the minor `hasDiff`-pollution FLAG). +3 regression tests. Gate green 171/1462.
- **RED-path test gap (MED):** the 5 regression tests mock execa returning only `{exitCode:0}` — they pin command *emission* (install-before-build order, `tsc -b` not `--noEmit`, lint scope, no `pnpm -r test`) but NEVER exercise a RED (no test that a failing typecheck yields `tscErrors≥1`, or that `reject:false` truly doesn't throw on install failure — the docstring's central safety claim is untested). Symmetric false-POSITIVE risk (gate silently swallowing a real RED) is unguarded. Add ≥2 failure-path tests.
- **Minor:** `--prefer-offline` (not `--frozen-lockfile`) can rewrite tracked `pnpm-lock.yaml` and dirty `git diff --stat HEAD` (`cast.ts:1071 hasDiff`) on dep drift.

**Recurrence:** 2026-05-30, RB2 Chunk 3 (cast-1780117396822) — SECOND consecutive build-before-gate cast false-negatived (`no_candidates_passed_gate` / A `test_gate`) while the curator's own `pnpm gate` was GREEN (170 files / 1454 tests, typecheck+lint+test EXIT=0) and the work was committed (`5bc237d`, reap-survival) → merged `3df454e`. Two manual overrides in two casts confirms the High severity; promoted to the top hardening follow-up (Task #9). The recurrence is the empirical case for fixing the scorer gate before the next `manta-cli`-touching cast rather than continuing to override by hand.

**Lessons:** an automated quality gate that does not reproduce the project's canonical gate (same commands, same prerequisites) gives false confidence in BOTH directions — here false-negative (good work discarded), but symmetrically it could false-positive (a branch that reds under `tsc -b` passing the scorer's weaker `tsc --noEmit`). The scorer must invoke the SAME `pnpm gate` the humans/CI trust, including its build prerequisite, or its verdict is uncomparable to the merge bar. Curator corollary (ceremony discipline): ALWAYS `cat docs/merge-reviews/cast-<id>.md` at step 1 — do not assume "no file was generated" from a prior note; the file here was untracked-present and was initially skipped.
...
- Note: the contract's "move `@manta/*` to devDependencies" was empirically WRONG — `pnpm pack` leaves them in the published devDependencies as `0.0.0`, and `npm i --omit=dev` (npm 10) still RESOLVES the dev tree → registry 404. A lifecycle-script manifest strip (`prepack`/`postpack`) IS viable on pnpm 9 — those hooks DO fire on `pnpm pack` (curator empirically confirmed against the sibling clone's working `prepack`/`postpack` script; pnpm 9 lacks only the newer `beforePacking` hook, which is 10.28+). The chosen fix instead drops the internal packages from the manifest entirely and resolves them at build time via tsup esbuild alias + tsconfig `paths` + vitest `resolve.alias` (all → sibling source/declarations). Rationale for preferring build-time inlining over a strip script: it is clean BY CONSTRUCTION — the published manifest never contains an unpublishable `@manta/*` in the first place, so correctness does not depend on a lifecycle hook firing and a `postpack` restore completing. A strip-on-pack script is conditional correctness — the same fragility class as bug #53 itself (a runtime/lifecycle hop that silently breaks the artifact if it doesn't run).
...
### #65 — plugin command-bin (`manta.cjs`) is not fully self-contained: dynamic `require.resolve('proper-lockfile')` escapes the tsup bundle → lock-using commands need `node_modules` at runtime

**Severity:** HIGH (PUBLISH-BLOCKER for the *command* path of the plugin; the bus path is unaffected). Found 2026-05-30, RB#3 cast `cast-1780171713628` clone A.

**Context:** RB#3 ships Manta as a Claude Code plugin. Plugins install by git-clone into the plugin cache with **no `npm install` / build step**, so the bundled bins must be fully self-contained. RB#3 added `packages/manta-cli/tsup.plugin.config.ts` (a `noExternal:[/.*/]` build, distinct from the npm build) so `build:plugin` emits inlined bins to `dist/bin/{manta.cjs,server.cjs}`.

**Reproducer (verified):** copy the committed `dist/bin/manta.cjs` to a dir with NO ancestor `node_modules`, then `CLAUDE_PLUGIN_ROOT=<dir> node manta.cjs --version` → `Error: Cannot find module 'proper-lockfile'` at a `Function.resolve` (require.resolve) call site (`manta.cjs:~41925`). `server.cjs` in the same isolation **passes** — it boots and answers MCP `tools/list` standalone. So the MCP bus (the plugin's zero-config auto-register win) works; the slash-command bin does not, for any command that touches the file lock (`cast`, `abort`, `kill`, `recover`).

**Root cause:** `proper-lockfile` is imported statically in `packages/manta-bus/src/atomic-fs.ts:3` (`import lockfile from 'proper-lockfile'`), but a runtime `require.resolve('proper-lockfile')` (proper-lockfile's own resolution of its lockfile helper, or an esbuild-emitted resolve fallback) is a **dynamic** specifier resolution that esbuild does NOT rewrite to a bundled module id — so it survives into the output and fails when there is no `node_modules`. This is the same fragility class as bug #53 (a runtime resolve hop that silently breaks the artifact when the surrounding tree isn't present).

**Why not fixed here:** the resolution site is in `@manta/bus` runtime (`atomic-fs.ts`) / the esbuild config's handling of it — **bus-runtime territory, explicitly out of this clone's RB#3 scope fence (that is RB#4).** Fixing blind (e.g. forcing CJS interop of an ESM-only transitive dep) risks breaking the verified-good `server.cjs`. Escalated to curator via `manta.broadcast` (`event_type: blocker`).
...
### #68 — no targeted way to clear settled (DEAD) clone records from `manta status`; `recover` doesn't, `cleanup` nukes everything

**Severity:** LOW-MEDIUM (UX). Found 2026-05-31 live (user noticed DEAD clones accumulating in `manta status` and asked how to clear / how to even get a clone id to kill).

**Two distinct UX gaps, one entry:**

1. **Clone-id discoverability.** `manta kill <id>` / `manta inspect <id>` need a clone id, but the only place it appears is the `Clone` column of `manta status` (the bare letters `A`/`B`/…), which is not labelled as "the id you pass to kill". A user reasonably asks "where do I get the clone id?". **Mitigation shipped (this commit):** `renderStatusTable` now prints an action-hint footer — when clones are live: `↑ "Clone" is the id. Stop one: manta kill <id> (e.g. manta kill A) · stop all: manta abort · details: manta inspect <id> [live: …]`. Reminder: `manta abort` stops ALL live clones with no id needed — the usual "stop everything" path.

2. **No DEAD-record purge.** Finished casts leave DEAD clone records in the registry; `manta status` keeps showing them. `manta recover` does **not** remove them (verified live: `0 reaped`, records persist — recover reaps *zombies*/stale claims/orphan worktrees, not correctly-DEAD records). The only thing that clears them is `manta cleanup` — which is a **full repo uninstall** (removes `.manta/worktrees/*`, `manta/cast-*` branches, the bus MCP registration, AND `.manta/state`), far more than "tidy the status list". DEAD records are bounded (max one per clone-letter A–E, overwritten when a letter is reused) so this is cosmetic, not growth. **Status-footer mitigation (this commit):** the all-DEAD footer now says finished casts are safe to ignore and that the next cast reuses the slots, instead of the earlier WRONG hint that `recover` clears them.

**Proper fix (follow-up cast):** add a targeted purge — either `manta recover --purge-dead` (drop DEAD records whose cast has fully settled) or a `manta status --clear-settled` — so the user can tidy the list without a full `cleanup`. Keep `cleanup` as the nuclear option.
