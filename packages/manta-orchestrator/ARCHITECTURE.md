...
- **State-ahead-of-audit window for reapers.** The reaper sequence is `store.reapX() → events.append(...)`: state mutation happens first, then the audit event. If the process crashes between the two, the lease/claim is gone but no `lock_reap` / `claim_reap` event was ever recorded. The next `runCycle` finds nothing to reap (idempotent), so the gap never widens, but a single audit-event-per-reap can be missed for that one window. This is documented symmetrically in `packages/manta-bus/ARCHITECTURE.md` under "Known invariants & limitations" and is an intentional Phase 0 carve-out — closing it requires a two-phase commit between the bus state file and the events log, which is out of scope until Phase 0d's CLI exposes a `manta audit` reconciler.
- **No `LocksStore.listAll`.** `buildStatus` aggregates lease lists across all *registered* clones via `LocksStore.listOwned(cloneId)`. Leases owned by a clone whose registry record is gone (zombies) won't show up in `getStatus()`; they surface as `lock_reap` events on the next cycle. Phase 0d will add `LocksStore.listAll` and `manta status` will pick it up automatically.
