...
- **Out of scope:**
  ...
  - Charge accounting (the orchestrator owns the charge ledger; the bus only logs deltas as events)
  - `forking-realities` Bus isolation
...
| `state/casts.ts` | Per-cast manifest: mode, roster, policy |
...
- **Append-only events log.** Every write goes through `events.append` so the orchestrator can replay or tail the log for post-mortem and observability. The orchestrator is responsible for emitting a `lock_reap` event after each call to `LocksStore.reapStale` that returns reaped leases, and a `claim_reap` event after each call to `ClaimsStore.reapExpired` that returns reaped claims — the stores themselves stay silent so the events log doesn't get littered with no-op cleanup ticks.
- **Registration-agnostic.** The bus does NOT enforce that a clone has called `manta.register` before invoking other tools. Clone lifecycle gating (refusing tool calls from un-registered or post-mortem clones) is the orchestrator's job. This keeps the bus a pure data plane and lets the orchestrator own lifecycle policy without coupling. The one exception is `manta.message`, which validates both `from_clone_id` and `to_clone_id` exist in the registry — because addressing a non-existent peer is a structural error, not a policy decision.
...
Readers: the bus sibling-messaging filter, the orchestrator (cast finalisation), and the CLI (replay/audit).
...
If step 3 throws, step 4 never runs — state is unchanged and the failure surfaces to the caller. The remaining race window is between a successful step 3 and a failed step 4 (e.g. disk full at rename); in that case the audit log is **ahead** of state. The orchestrator detects and reconciles such drift on replay.
...
**Reaper exception class.** `LocksStore.reapStale` and `ClaimsStore.reapExpired` intentionally do **not** accept an `auditAppend` callback. The orchestrator emits `lock_reap` / `claim_reap` events *after* the store call returns the list of reaped entries, so audit-versus-state ordering is inverted: state mutates first, then the audit event lands. This introduces a small **state-ahead-of-audit** window (the inverse of the normal audit-ahead-of-state window). The orchestrator's `runCycle` retry contract handles this by virtue of being idempotent on already-reaped state — a crash between reap and audit-append leaves the next cycle observing zero stale entries, which is the correct steady-state.
...
- **State-ahead-of-audit (reapers only).** `LocksStore.reapStale` and `ClaimsStore.reapExpired` mutate state first, then return the reaped list to the orchestrator, which appends `lock_reap` / `claim_reap` events outside the store mutex. A crash in that gap leaves state freshly reaped but no audit entry. The orchestrator's reaper cycles are idempotent (already-reaped entries are not re-reaped) so this is reconciled by virtue of running again — there is no replay step needed.
