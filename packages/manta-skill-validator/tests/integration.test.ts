...
describe('Phase 0e content integration', () => {
  ...
  it('all skills are present', async () => {
    ...
    expect(skills).toEqual(['manta-as-clone', 'manta-cast-decide', 'manta-coordinate', 'manta-daemon-idle', 'manta-doc-chase', 'manta-graceful-death', 'manta-merge-review', 'manta-pair-protocol', 'manta-pair-reviewer', 'manta-pair-writer', 'manta-storm-coder', 'manta-storm-fuzzer', 'manta-storm-tester']);
  });
  ...
  it('all slash commands are present', async () => {
    ...
    expect(commands).toEqual(['abort', 'cast', 'cost', 'kill', 'promote', 'recover', 'status']);
  });
});
