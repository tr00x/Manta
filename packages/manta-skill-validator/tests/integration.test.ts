...
describe('Phase 0e content integration', () => {
  ...
  it('all skills are present', async () => {
    ...
    expect(skills).toEqual(['manta-as-clone', 'manta-cast-decide', 'manta-coordinate', 'manta-council', 'manta-daemon-idle', 'manta-decoy', 'manta-doc-chase', 'manta-graceful-death', 'manta-merge-review', 'manta-orchestrate', 'manta-pair-protocol', 'manta-pair-reviewer', 'manta-pair-writer', 'manta-storm-coder', 'manta-storm-fuzzer', 'manta-storm-tester']);
  });
  ...
  it('all slash commands are present', async () => {
    ...
    expect(commands).toEqual(['abort', 'cast', 'doctor', 'help', 'inspect', 'kill', 'promote', 'recover', 'replay', 'status', 'tail']);
  });
});
