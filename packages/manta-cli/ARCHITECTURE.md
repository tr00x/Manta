...
- **Out of scope:**
  ...
  - Skills + slash commands — owned by `manta` consumers
  - Plugin distribution
...
- **CloneRunner is the test seam.** Production = `runClaudeCli` (spawns `claude --print`). Tests = `runFakeCloneScript` (spawns a Node fixture). Same code path through `spawnClone`, no `if env === "prod" else mock` — one code path, DI seam only.
...
- **Cumulative cost gate.** Per-clone × cloneCount must not exceed the per-cast usage ceiling (a token-estimate proxy, not dollars). The charge / cooldown system enforces frequency on top.
...
- **Worktrees stay after a cast.** Manta keeps `clone-` worktrees on disk so the operator can `cd` in and inspect post-mortem state; `manta abort` manages retention.
