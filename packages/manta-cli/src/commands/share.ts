  ...
  try {
    for (const file of await fs.readdir(modesDir)) {
      ...
      if (parsed && typeof parsed === 'object') {
        ...
        if (typeof m.name === 'string' && typeof m.description === 'string' && typeof m.basedOn === 'string') {
          contributes.modes.push({
            name: m.name,
            description: m.description,
            basedOn: m.basedOn as never,
            cloneCount: (m.cloneCount as { min: number; max: number }) ?? { min: 1, max: 1 },
            sessionMode: (m.sessionMode as 'batch' | 'daemon') ?? 'batch',
            templates: [],
            ...(typeof m.capabilityProfile === 'string'
