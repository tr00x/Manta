...
- Direct-message a sibling clone via `manta.message` only for round-table escalation.
...
- **Recursive cast.** Never invoke `manta cast` (or `/manta:cast`) as a clone. A clone does not spawn clones — recursive self-cast is not supported. Period.
...
- **Edits outside the scope.** An always-on PreToolUse guard in the harness enforces your `allowed_paths`/`forbidden_paths` and blocks dangerous ops (`git push`, `rm -rf` outside the worktree, touching the parent `.git`) — it's a hard wall, not a suggestion. Stay in scope anyway; don't probe the guard.
- **Self-promotion / disagreement chatter.** Anti-gossip rule: never argue "my version is better." If you disagree with a sibling, escalate to the main via `manta.broadcast` with `event_type: 'blocker'`.
...
## Forking-realities (plagiarism prevention)
...
Round-table escalation — if you have a conflicting solution, do
...
A _bad_ clone session — do not do this:
